#!/usr/bin/env python3
"""
ents_cluster_server_V3
Based on ents_cluster_server_X1
Original: Maria Katherine
This version: Satej

Adds single schema table data output with the existing log.
Features added in V3:
- log_data() writes one real CSV row per measurement (power or bme280)
- current_pot_value tracked and tagged on every row
- data_filename kept separate from the existing status/log file (savefile), nothing about the original logging changes
"""
...
voltage = 0.0
current = 0.0
current_pot_value = None  # tracks which pot step is active right now, for tagging data rows
...
data_filename = "ents_cluster_server_" + f"{timestamp}_" + userfilename + "_data.csv"


def initialize_csv():
    with open(data_filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["local_time", "logger_id", "pot_value", "type", "voltage", "current", "power", "temperature", "humidity"])
...
def start_timer_pot(pot):
    global timer_started, timer, pot_cycle_complete, pot_count, voltage, current, current_pot_value
    log_excel("Voltage threshold reached. Starting timer.")
    time.sleep(2)
    send_command(f"pot {pot}")
    current_pot_value = pot
    time.sleep(timer * 60)
...
def receive_data():
    ...
    try:
        ...
        if meas['type'] == 'power':
            ...
            log_data([local_time, logger_id, current_pot_value, "power", voltage, current, voltage*current, None, None])
            ...
            local_time = datetime.datetime.now().strftime('%H:%M:%S')
            log_data([local_time, logger_id, current_pot_value, "bme280", None, None, None, temperature, humidity])
