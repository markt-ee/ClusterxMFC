...
void ModuleWiFi::Connect(const Esp32Command &cmd) {
  ...
  // //set config to static IP
  // IPAddress local_IP(192, 168, 0, 219);     // Chosen static IP for ESP32
  // IPAddress gateway(192, 168, 0, 1);        // Same as your router
  // IPAddress subnet(255, 255, 255, 0);       // Matches your subnet mask
  // //IPAddress dns(8, 8, 8, 8);                // Google's DNS (or your own) connects to the internet- remove
...
void ModuleWiFi::Time(const Esp32Command &cmd) {
  ...
  if (wifi_status != WL_CONNECTED) {
    ...
    wifi_cmd.ts = millis()/1000; //rough time stamp
    ...
    //   Log.errorln("Failed to get time from millis");
    // }
  }
