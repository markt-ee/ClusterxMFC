...
- need to include the first ocv value as part of the plot 
"""
...
SAVEFILE = f"entscluster_V2_{TIMESTAMP_STR}_{USER_FILENAME}.csv"
FILEDIR = Path("/home/avocado/ClusterxMFC/ents_cluster_server/ents_cluster_server_V2/")
...
def initialize_csv():
    with open(CSV_PATH, mode="w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["Timestamp", "local ts", "logger id", "Current", "Voltage", "Temperature", "Humidity", "Power Density(mW/m^2)", "Log", "error"])
...
#At Lab Setup
ents13 = EntsLogger(13, "Luke",  "192.168.0.147", pot_en=False, mppt_en=True, ocv=0.7)
ents11 = EntsLogger(11, "Leia",  "192.168.0.116", pot_en=False, mppt_en=True, ocv=0.7)
...
def generate_png(currents_mA, voltages_mV, power_uW, logger: EntsLogger):
    ...
    power_density = (power_uW/1000)/0.00567 #mW/m^2
    current_density = (currents_mA)/0.00567 #mA/m^2
    ax1.plot(current_density, voltages_mV, marker='o', linestyle='-', color='blue', label='Voltage')
    ax1.set_xlabel("Current Density (mA/m^2)")
    ax1.set_ylabel("Voltage (mV)")
    ax1.grid(True)

    ax2 = ax1.twinx()
    ax2.plot(current_density, power_density, marker='s', linestyle='-', color='red', label='Power')
    ax2.set_ylabel("Power Density(mW/m^2)")
...
def start_timer_pot(pot_index: int, pot_value: int, logger: EntsLogger):
    ...
    try:
        ...
        if cycle_complete:
            ...
            power_density = (power_uW/1000)/0.00567 #mW/m^2
            current_density = (i_arr*1000)/0.00567 #mA/m^2
            ...
            log_excel(f"[logger {logger.id}] Cycle complete. voltage_mV = {v_arr}\n current_mA = {i_arr}\n power_uW: {power_uW}\n power_density: {power_density}\n current_density: {current_density}")
